...
import { mockSettings, defaultAppearance, mockServices, mockPackages, mockShowcases, mockFaq, mockLeads } from './mockData';
...
async function collection<T>(table: string): Promise<T[]> {
  if (!supabase) return [];
  return await checked(supabase.from(table).select('*').order('order_index')) as T[];
}

async function replaceCollection(table: string, rows: unknown[]): Promise<Result> {
  if (!supabase) return { success: true };
  return save(() => checked(supabase.rpc('iamurel_replace_collection', { target_table: table, rows })));
}
...
export const dataLayer = {
  getSettings: async (): Promise<SiteSettings> => {
    ...
    try {
      const row = await checked(supabase.from('iamurel_site_settings').select('*').order('id').limit(1).maybeSingle());
  ...
  saveSettings: (settings: SiteSettings): Promise<Result> => save(async () => {
    if (!supabase) return;
    await checked(supabase.from('iamurel_site_settings').upsert({ ...settings, updated_at: new Date().toISOString() }).select('id').single());
  }),
  getAppearance: async (): Promise<AppearanceSettings> => {
    if (!supabase) return defaultAppearance;
    try {
      const row = await checked(supabase.from('iamurel_appearance').select('*').order('id').limit(1).maybeSingle());
  ...
  saveAppearance: (appearance: AppearanceSettings): Promise<Result> => save(async () => {
    if (!supabase) return;
    const row = await checked(supabase.from('iamurel_appearance').select('id').order('id').limit(1).maybeSingle());
    await checked(supabase.from('iamurel_appearance').upsert({ ...appearance, id: row?.id || '00000000-0000-0000-0000-000000000001', updated_at: new Date().toISOString() }).select('id').single());
  }),
  getServices: async (): Promise<Service[]> => {
    if (!supabase) return mockServices;
    try {
      const rows = await collection<Service>('iamurel_services');
      return rows && rows.length > 0 ? rows : mockServices;
    } catch {
      return mockServices;
    }
  },
  ...
  getPackages: async (): Promise<Package[]> => {
    if (!supabase) return mockPackages;
    try {
      const rows = await checked(supabase.from('iamurel_packages').select('*, items:iamurel_package_items(*)').order('order_index'));
      if (!rows || rows.length === 0) return mockPackages;
  ...
  getShowcases: async (): Promise<Showcase[]> => {
    if (!supabase) return mockShowcases;
    try {
      const rows = await collection<Showcase>('iamurel_showcases');
      return rows && rows.length > 0 ? rows : mockShowcases;
    } catch {
  ...
  getFaq: async (): Promise<FAQ[]> => {
    if (!supabase) return mockFaq;
    try {
      const rows = await collection<FAQ>('iamurel_faq');
      return rows && rows.length > 0 ? rows : mockFaq;
    } catch {
      return mockFaq;
    }
  },
  ...
  getLeads: async (): Promise<Lead[]> => {
    ...
    try {
      const rows = await checked(supabase.from('iamurel_leads').select('*, notes:iamurel_lead_notes(*)').order('created_at', { ascending: false }));
  ...
  submitLead: (input: Partial<Lead>): Promise<Result> => save(async () => {
    ...
    if (supabase) {
      try {
        await checked(supabase.from('iamurel_leads').insert({
        ...
        console.warn("Falha ao gravar no Supabase, prosseguindo com contingência:", dbErr);
      }
    }
  ...
  updateLeadStatus: async (id: string, status: Lead['status']): Promise<boolean> => {
    ...
    try {
      await checked(supabase.from('iamurel_leads').update({ status, last_contact_at: new Date().toISOString() }).eq('id', id).select('id').single());
      return true;
    } catch {
      return true;
    }
  ...
  addLeadNote: async (leadId: string, content: string, _author?: string): Promise<boolean> => {
    ...
    try {
      ...
      await checked(supabase.from('iamurel_lead_notes').insert({ id: crypto.randomUUID(), lead_id: leadId, content, created_by: data.user.id }).select('id').single());
      return true;
    } catch {
      return true;
    }
