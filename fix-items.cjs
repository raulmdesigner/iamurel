...
code = code.replace(itemsEditor, '');

// Now we need to add it to packages. 
// We will look for the end of the package fields, right before closing the div of the package.
// Let's find a reliable anchor in the pacotes section.
const packageAnchor = `                        className="w-full p-2 bg-bg border border-border rounded text-xs"
                      />
                    </div>
                  </div>
                </div>`;

const replaceWith = packageAnchor + '\n' + itemsEditor;

code = code.replace(packageAnchor, replaceWith);
